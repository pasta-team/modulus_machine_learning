...
for i in dirs:
    ...
    for j in val:
        ...
        for l,j in zip(ele,no_data):
            ene -= energy[l]*j
        epa = ene/no_atoms
        data.append([[lg_vpa,epa,row_no,col_no,chi,vd,pg_ord,int(i),no_data,ld[int(i)]],math.log(KH),math.log(GH),s])
        #try:
        #    rdf = s.rdf_for_np()
        #except:
        #    continue
        #res = [lg_vpa,epa,row_no,col_no,chi,vd,pg_ord,int(i),no_data,ld[int(i)]]
        #res.extend(rdf)
        #data.append((res,math.log(KH),math.log(GH)))

with open('./symelasticity','wb') as f:
    pickle.dump(data,f)
